...
import { severityColor, severityLabel, signColor, fmtDate }
  from '../clube/utils/formatters.js';
...
export default function ClubeRiscoPage() {
  ...
  useEffect(() => {
    ...
    async function fetchAll() {
      ...
      try {
        ...
        const [clubeRes, navRes, opRes, posRes, compRes] = await Promise.allSettled([
          ...
          fetch(`${API}/api/v1/clubes/${clubeIdParam}/compliance`,         { headers }),
        ]);
  ...
  // Sorted posicoes by peso_alvo DESC for "Maior Posição" card
  const sortedPosicoes = useMemo(() =>
    [...posicoes].sort((a, b) => (b.peso_alvo ?? 0) - (a.peso_alvo ?? 0)),
    [posicoes]);

  // Compliance display derived values
