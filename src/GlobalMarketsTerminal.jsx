...
import { ExchangeBadge, SourceBadge, MacroBanner, CrossListBadge } from './components/gmtBadges.jsx';
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 1400, margin: "0 auto", padding: "0 20px 40px" }}>
        ...
        {!loading && marketData && (
          <div className="fade-in">
            ...
            {/* ── CARD VIEWS (watchlist / flatExpand / collapsed / normal expanded) ── */}
            {viewMode !== "list" && (watchlistFlatSymbols ? (
              ...
              <div style={{ display: "grid", gridTemplateColumns: `repeat(auto-fill, minmax(${(DENSITY_CONFIG[cardDensity] || DENSITY_CONFIG.compact).gridMin}px, 1fr))`, gap: (DENSITY_CONFIG[cardDensity] || DENSITY_CONFIG.compact).gap, padding: "12px 0", transition: "all 0.2s ease" }}>
                {sortedCats.map((catKey) => {
                  ...
                  if (!isOpen) {
                    return <GroupSummaryCard key={catKey} catKey={catKey} cat={cat} layout="grid" onExpand={() => toggleCard(catKey)} density={cardDensity} getGroupSymbols={getGroupSymbols} marketData={marketData} assets={ASSETS} />;
                  }
                  ...
                  return (
                    <div key={catKey} className="section-animate" style={{ marginBottom: collapsedGroups[catKey] ? 8 : 24, animationDelay: `${idx * 0.08}s` }}>
                      {collapsedGroups[catKey] ? (
                        <GroupSummaryCard catKey={catKey} cat={cat} layout="row" onExpand={() => toggleGroup(catKey)} getGroupSymbols={getGroupSymbols} marketData={marketData} assets={ASSETS} />
                      ) : (
                        <>
                          <div
